/**
 * Prompt guidance contract validator.
 *
 * Validates AGENTS.md content against required guidance fragments per the
 * OMGHC guidance schema (executor-constraints, planner-investigation,
 * verifier-shared, etc.). Also exports the contract registry inherited from
 * OMX (rename pass: omx -> omghc) so that copilot-native-hook.ts and tests
 * can introspect required pattern sets per surface.
 */

import { existsSync, readFileSync } from "node:fs";

// ---------------------------------------------------------------------------
// Public schema (per task #3 spec)
// ---------------------------------------------------------------------------

export interface GuidanceFragmentRef {
  name: string;
  required: boolean;
  source?: string;
}
...
/**
 * Validate that an AGENTS.md (or similar) document satisfies the supplied
 * fragment requirements. A fragment is considered present when the document
 * contains either a literal occurrence of the fragment name, or — when the
 * `source` path is provided — a substantial slice of that source file's text.
 *
 * Required fragments raise an `error` issue when missing.
 * Optional fragments raise a `warn` issue when missing.
 * Unknown source paths raise a single `info` issue (validation still proceeds).
 */
export function validateGuidance(
  content: string,
  requiredFragments: GuidanceFragmentRef[],
): ValidationResult {
  ...
  if (typeof content !== "string") {
    return {
      valid: false,
      issues: [{ severity: "error", message: "guidance content must be a string" }],
    };
  }

  for (const frag of requiredFragments) {
    const present = fragmentPresent(content, frag, issues);
    if (!present) {
      issues.push({
        severity: frag.required ? "error" : "warn",
        message: frag.required
          ? `required guidance fragment missing: ${frag.name}`
          : `optional guidance fragment missing: ${frag.name}`,
        line: locateFragmentMarker(content, frag.name),
      });
    }
  }
...
function fragmentPresent(
  content: string,
  frag: GuidanceFragmentRef,
  issues: ValidationIssue[],
): boolean {
  if (containsFragmentMarker(content, frag.name)) {
    return true;
  }
  ...
  if (frag.source) {
    if (!existsSync(frag.source)) {
      issues.push({
        severity: "info",
        message: `guidance fragment source not found on disk: ${frag.source}`,
      });
      return false;
    }
    let sourceText: string;
    try {
      sourceText = readFileSync(frag.source, "utf8");
    } catch (err) {
      issues.push({
        severity: "info",
        message: `failed to read guidance fragment source ${frag.source}: ${err instanceof Error ? err.message : String(err)}`,
      });
      return false;
    }
    return contentIncludesSlice(content, sourceText);
  }

  return false;
}

function containsFragmentMarker(content: string, name: string): boolean {
...
function locateFragmentMarker(content: string, name: string): number | undefined {
...
function contentIncludesSlice(content: string, sourceText: string): boolean {
  const trimmed = sourceText.trim();
  if (trimmed.length === 0) return true;
  const slice = trimmed.length > 80 ? trimmed.slice(0, 80) : trimmed;
  return content.includes(slice);
}

// ---------------------------------------------------------------------------
// Contract registry (ported from OMX; rename pass omx -> omghc)
// ---------------------------------------------------------------------------
...
const ROOT_TEMPLATE_PATTERNS = [
  ...
  rx("OMGHC runtime manipulation.*agent responsibilities"),
...
export const PROMPT_REFACTOR_INVARIANT_CONTRACTS: GuidanceSurfaceContract[] = [
  ...
  },
  {
    ...
    requiredPatterns: [rx("omghc\\s+question"), rx("Socratic|interview"), rx("ambiguity")],
  },
  {
