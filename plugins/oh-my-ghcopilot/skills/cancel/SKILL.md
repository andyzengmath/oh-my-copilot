---
name: cancel
description: Cancel any active OMGHC mode (autopilot, ralph, ultrawork, ecomode, ultraqa, swarm, ultrapilot, pipeline, team)
---

# Cancel Skill

Intelligent cancellation that detects and cancels the active OMGHC mode.

**The cancel skill is the standard way to complete and exit any OMGHC mode.**
...
- By default the command inspects the current session via `state_list_active` and `state_get_status`, navigating `.omghc/state/sessions/{sessionId}/…` to discover which mode is active.
- When a session id is provided or already known, that session-scoped path is authoritative. Legacy files in `.omghc/state/*.json` are consulted only as a compatibility fallback if the session id is missing or empty.
- Swarm is a shared SQLite/marker mode (`.omghc/state/swarm.db` / `.omghc/state/swarm-active.marker`) and is not session-scoped.
...
1. `state_list_active` enumerates `.omghc/state/sessions/{sessionId}/…` to find every known session.
2. `state_clear` runs once per session to drop that session’s files.
3. A global `state_clear` without `session_id` removes legacy files under `.omghc/state/*.json`, `.omghc/state/swarm*.db`, and compatibility artifacts (see list).
4. Team artifacts (`.omghc/state/team/*/`, tmux sessions matching `omghc-team-*`) are best-effort cleared as part of the legacy fallback.

Every `state_clear` command honors the `session_id` argument, so even force mode still uses the session-aware paths first before deleting legacy files.

Legacy compatibility list (removed only under `--force`/`--all`):
- `.omghc/state/autopilot-state.json`
- `.omghc/state/ralph-state.json`
- `.omghc/state/ralph-plan-state.json`
- `.omghc/state/ralph-verification.json`
- `.omghc/state/ultrawork-state.json`
- `.omghc/state/ecomode-state.json`
- `.omghc/state/ultraqa-state.json`
- `.omghc/state/swarm.db`
- `.omghc/state/swarm.db-wal`
- `.omghc/state/swarm.db-shm`
- `.omghc/state/swarm-active.marker`
- `.omghc/state/swarm-tasks.db`
- `.omghc/state/ultrapilot-state.json`
- `.omghc/state/ultrapilot-ownership.json`
- `.omghc/state/pipeline-state.json`
- `.omghc/state/plan-consensus.json`
- `.omghc/state/ralplan-state.json`
- `.omghc/state/boulder.json`
- `.omghc/state/hud-state.json`
- `.omghc/state/subagent-tracking.json`
- `.omghc/state/subagent-tracker.lock`
- `.omghc/state/rate-limit-daemon.pid`
- `.omghc/state/rate-limit-daemon.log`
- `.omghc/state/checkpoints/` (directory)
- `.omghc/state/sessions/` (empty directory cleanup after clearing sessions)
...
1. Call `state_list_active` to enumerate `.omghc/state/sessions/{sessionId}/…` and discover every active session.
...
3. If a `session_id` was supplied to `/cancel`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.omghc/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
...
Teams are detected by checking for config files in `.omghc/state/team/`:

```bash
# Check for active teams
ls .omghc/state/team/*/config.json 2>/dev/null
```
...
```
For each team found in .omghc/state/team/:
  1. Read config.json to get team_name and workers list
  2. For each worker:
     a. Write shutdown inbox to .omghc/state/team/{name}/workers/{worker}/inbox.md
...
After graceful pass:
  ...
  2. Destroy the tmux session: tmux kill-session -t omghc-team-{name}
```

**Cleanup:**
```
  1. Strip AGENTS.md team worker overlay (<!-- OMGHC:TEAM:WORKER:START/END -->)
  2. Remove team state directory: rm -rf .omghc/state/team/{name}/
...
1. Check `.omghc/state/team/*/config.json` for active teams
...
5. Destroy tmux session: `tmux kill-session -t omghc-team-{name}`
6. Strip AGENTS.md overlay
7. Remove state: `rm -rf .omghc/state/team/{name}/`
...
if [[ -f .omghc/state/autopilot-state.json ]]; then
  # Clean up ralph if active
  if [[ -f .omghc/state/ralph-state.json ]]; then
    RALPH_STATE=$(cat .omghc/state/ralph-state.json)
    LINKED_UW=$(echo "$RALPH_STATE" | jq -r '.linked_ultrawork // false')

    # Clean linked ultrawork first
    if [[ "$LINKED_UW" == "true" ]] && [[ -f .omghc/state/ultrawork-state.json ]]; then
      rm -f .omghc/state/ultrawork-state.json
      echo "Cleaned up: ultrawork (linked to ralph)"
    fi

    # Clean ralph
    rm -f .omghc/state/ralph-state.json
    rm -f .omghc/state/ralph-verification.json
    echo "Cleaned up: ralph"
  fi

  # Clean up ultraqa if active
  if [[ -f .omghc/state/ultraqa-state.json ]]; then
    rm -f .omghc/state/ultraqa-state.json
    echo "Cleaned up: ultraqa"
  fi

  # Mark autopilot inactive but preserve state
  CURRENT_STATE=$(cat .omghc/state/autopilot-state.json)
  CURRENT_PHASE=$(echo "$CURRENT_STATE" | jq -r '.phase // "unknown"')
  echo "$CURRENT_STATE" | jq '.active = false' > .omghc/state/autopilot-state.json
...
if [[ -f .omghc/state/ralph-state.json ]]; then
  ...
  # Clean linked ultrawork first
  if [[ "$LINKED_UW" == "true" ]] && [[ -f .omghc/state/ultrawork-state.json ]]; then
    UW_STATE=$(cat .omghc/state/ultrawork-state.json)
  ...
  # Clean ralph state
  rm -f .omghc/state/ralph-state.json
  rm -f .omghc/state/ralph-plan-state.json
  rm -f .omghc/state/ralph-verification.json
...
```bash
if [[ -f .omghc/state/ultrawork-state.json ]]; then
...
```bash
echo "No active OMGHC modes detected."
echo ""
echo "Checked for:"
echo "  - Autopilot (.omghc/state/autopilot-state.json)"
echo "  - Ralph (.omghc/state/ralph-state.json)"
echo "  - Ultrawork (.omghc/state/ultrawork-state.json)"
echo "  - UltraQA (.omghc/state/ultraqa-state.json)"
echo ""
...
4. In force mode, iterate every active session, call `state_clear` per session, then run a global `state_clear` without `session_id` to drop legacy files (`.omghc/state/*.json`, compatibility artifacts) and report success. Swarm remains a shared SQLite/marker mode outside session scoping.
5. Team artifacts (`.omghc/state/team/*/`, tmux sessions matching `omghc-team-*`) remain best-effort cleanup items invoked during the legacy/global pass.
...
| Force | "All OMGHC modes cleared. You are free to start fresh." |
| None | "No active OMGHC modes detected." |
...
- **Local-only**: Clears state files in `.omghc/state/` directory
...
1. **Kill all team tmux sessions**: `tmux list-sessions -F '#{session_name}' 2>/dev/null | grep '^omghc-team-'` and kill each
2. **Remove team state directories**: `rm -rf .omghc/state/team/*/`
3. **Strip AGENTS.md overlay**: Remove content between `<!-- OMGHC:TEAM:WORKER:START -->` and `<!-- OMGHC:TEAM:WORKER:END -->`

### Force Clear Addition

When `--force` is used, also clean up:
```bash
rm -rf .omghc/state/team/                  # All team state
# Kill all omghc-team-* tmux sessions
tmux list-sessions -F '#{session_name}' 2>/dev/null | grep '^omghc-team-' | while read s; do tmux kill-session -t "$s" 2>/dev/null; done
```
