...
0. **Pre-context intake (required before planning/execution loop starts)**:
   - Assemble or load a context snapshot at `.omghc/context/{task-slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`).
   ...
   - If request ambiguity is high, gather brownfield facts first. When session guidance enables `USE_OMGHC_EXPLORE_CMD`, prefer `omghc explore` for simple read-only repository lookups with narrow, concrete prompts; otherwise use the richer normal explore path. Then run `$deep-interview --quick <task>` to close critical gaps.
...
5. **Visual task gate (when screenshot/reference images are present)**:
   ...
   - Persist verdict to `.omghc/state/{scope}/ralph-progress.json` including numeric + qualitative feedback.
...
7.5 **Mandatory Deslop Pass**:
   - After Step 7 passes, run `oh-my-ghcopilot:ai-slop-cleaner` on **all files changed during the Ralph session**.
...
- Use `ask_copilot` with `agent_role: "architect"` for verification cross-checks when changes are security-sensitive, architectural, or involve complex multi-system integration
- Skip Copilot consultation for simple feature additions, well-tested changes, or time-critical verification
- If ToolSearch finds no MCP tools or Copilot is unavailable, proceed with architect agent verification alone -- never block on external tools
...
- If an `omghc_state` MCP tool call reports that its stdio transport is unavailable/closed, do **not** retry the same MCP call. Retry once through the supported CLI parity surface with the same payload, preserving `workingDirectory` and `session_id`: `omghc state write --input '<json>' --json`, `omghc state read --input '<json>' --json`, or `omghc state clear --input '<json>' --json`. If the CLI path also fails, continue with `.omghc/context` / `.omghc/plans` file-backed artifacts and report the state persistence blocker.
</Tool_Usage>

## State Management

Use the `omghc_state` MCP server tools (`state_write`, `state_read`, `state_clear`) for Ralph lifecycle state.
...
Prompt-side `$ralph` workflow activation is lighter-weight than `omghc ralph --prd ...`.
It seeds Ralph workflow state and guidance, but it does not implicitly launch the
CLI entrypoint or apply the PRD startup gate. Treat `omghc ralph --prd ...` as the
...
1. Run deep-interview in quick mode before creating PRD artifacts:
   ...
   - Persist interview output to `.omghc/interviews/{slug}-{timestamp}.md`
2. Create canonical PRD/progress artifacts:
   - PRD: `.omghc/plans/prd-{slug}.md`
   - Progress ledger: `.omghc/state/{scope}/ralph-progress.json` (session scope when available, else root scope)
...
5. Initialize canonical progress ledger at `.omghc/state/{scope}/ralph-progress.json`
...
Workflow: Detect flag, extract task, create `.omghc/plans/prd-{slug}.md`, create `.omghc/state/{scope}/ralph-progress.json`, begin ralph loop.

### Legacy compatibility
- During the compatibility window, Ralph `--prd` startup still validates machine-readable story state from `.omghc/prd.json`.
- `.omghc/plans/prd-{slug}.md` remains the canonical storage/documentation artifact, but it is not yet the startup validation source.
- If `.omghc/prd.json` exists and canonical PRD is absent, migrate one-way into `.omghc/plans/prd-{slug}.md`.
- If `.omghc/progress.txt` exists and canonical progress ledger is absent, import one-way into `.omghc/state/{scope}/ralph-progress.json`.
