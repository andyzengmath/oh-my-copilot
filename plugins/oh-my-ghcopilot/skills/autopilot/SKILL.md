...
0. **Pre-context Intake (required before Phase 0 starts)**:
   ...
   - Load the latest relevant snapshot from `.omghc/context/{slug}-*.md` when available.
   - If no snapshot exists, create `.omghc/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) with:
     - Task statement
     - Desired outcome
...
1. **Phase 0 - Expansion**: Turn the user's idea into a detailed spec
   - If `.omghc/specs/deep-interview-*.md` exists for this task: reuse it and skip redundant expansion work
   ...
   - Output: `.omghc/plans/autopilot-spec.md`
...
2. **Phase 1 - Planning**: Create an implementation plan from the spec
   ...
   - Output: `.omghc/plans/autopilot-impl.md`
...
6. **Phase 5 - Cleanup**: Clear all mode state via OMGHC MCP tools on successful completion
...
Use `omghc_state` MCP tools for autopilot lifecycle state.
