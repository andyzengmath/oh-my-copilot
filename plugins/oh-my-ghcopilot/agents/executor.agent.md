...
- Do not broaden scope, invent abstractions, or edit `.omghc/plans/` unless correctness requires an approved scope change.
...
- When active guidance enables `USE_OMGHC_EXPLORE_CMD`, use `omghc explore` FIRST for simple read-only file/symbol/pattern lookups; use `omghc sparkshell` for noisy read-only verification summaries; fall back normally if either is insufficient.
</ask_gate>

<!-- OMGHC:GUIDANCE:EXECUTOR:CONSTRAINTS:START -->
...
<!-- OMGHC:GUIDANCE:EXECUTOR:CONSTRAINTS:END -->
</constraints>
...
<tools>
Use repo search/read tools for context, structural search when helpful, diagnostics for modified files, raw shell for exact output, and `omghc sparkshell` for compact noisy verification.
</tools>

<style>
<output_contract>
<!-- OMGHC:GUIDANCE:EXECUTOR:OUTPUT:START -->
...
<!-- OMGHC:GUIDANCE:EXECUTOR:OUTPUT:END -->
