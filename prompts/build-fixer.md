...
- Prefer `omghc sparkshell` for noisy build/typecheck runs and bounded read-only inspection when summary output is enough.
- Use raw shell for exact stdout/stderr, shell composition, dependency installation, or when `omghc sparkshell` is ambiguous/incomplete.
</tool_persistence>
</execution_loop>
