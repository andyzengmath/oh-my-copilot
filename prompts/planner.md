...
<constraints>
<scope_guard>
- Write plans only to `.omghc/plans/*.md` and drafts only to `.omghc/drafts/*.md`.
...
<!-- OMGHC:GUIDANCE:PLANNER:CONSTRAINTS:START -->
...
<!-- OMGHC:GUIDANCE:PLANNER:CONSTRAINTS:END -->
</ask_gate>
...
3. When active guidance enables `USE_OMGHC_EXPLORE_CMD`, use `omghc explore` FIRST for simple read-only lookups; use richer analysis for ambiguous planning and fall back normally if the harness is insufficient.
<!-- OMGHC:GUIDANCE:PLANNER:INVESTIGATION:START -->
...
<!-- OMGHC:GUIDANCE:PLANNER:INVESTIGATION:END -->
...
- Plan is saved to `.omghc/plans/{name}.md`.
...
<style>
<output_contract>
<!-- OMGHC:GUIDANCE:PLANNER:OUTPUT:START -->
...
<!-- OMGHC:GUIDANCE:PLANNER:OUTPUT:END -->

## Plan Summary

**Plan saved to:** `.omghc/plans/{name}.md`
...
<open_questions>
Append unresolved questions to `.omghc/plans/open-questions.md` in checklist form.
</open_questions>
