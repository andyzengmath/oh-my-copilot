---
name: worker
description: Team worker protocol (ACK, mailbox, task lifecycle) for tmux-based OMGHC teams
---

# Worker Skill

This skill is for a Copilot session that was started as an OMGHC Team worker (a tmux pane spawned by `$team`).

## Identity

You MUST be running with `OMGHC_TEAM_WORKER` set. It looks like:
...
1. Parse `OMGHC_TEAM_WORKER` into:
   - `teamName` (before the `/`)
...
Use CLI interop:
- `omghc team api send-message --input <json> --json` with `{team_name, from_worker, to_worker:"leader-fixed", body}`

Copy/paste template:

```bash
omghc team api send-message --input "{\"team_name\":\"<teamName>\",\"from_worker\":\"<workerName>\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: <workerName> initialized\"}" --json
```

## Inbox + Tasks

1. Resolve canonical team state root in this order:
   1) `OMGHC_TEAM_STATE_ROOT` env
   2) worker identity `team_state_root`
   3) team config/manifest `team_state_root`
   4) local cwd fallback (`.omghc/state`)
2. Read your inbox:
...
6. Claim the task (do NOT start work without a claim) using claim-safe lifecycle CLI interop (`omghc team api claim-task --json`).
7. Do the work.
8. Complete/fail the task via lifecycle transition CLI interop (`omghc team api transition-task-status --json`) from `in_progress` to `completed` or `failed`.
   - Do NOT directly write lifecycle fields (`status`, `owner`, `result`, `error`) in task files.
9. Use `omghc team api release-task-claim --json` only for rollback/requeue to `pending` (not for completion).
...
Use CLI interop:
- `omghc team api mailbox-list --json` to read
- `omghc team api mailbox-mark-delivered --json` to acknowledge delivery

Copy/paste templates:

```bash
omghc team api mailbox-list --input "{\"team_name\":\"<teamName>\",\"worker\":\"<workerName>\"}" --json
omghc team api mailbox-mark-delivered --input "{\"team_name\":\"<teamName>\",\"worker\":\"<workerName>\",\"message_id\":\"<MESSAGE_ID>\"}" --json
```
...
- Prefer inbox/mailbox/task state and `omghc team api ... --json` operations.
