...
The security-reviewer agent SHOULD consult an external advisor for cross-validation when available.

### Protocol
1. **Form your OWN security analysis FIRST** - Complete the review independently
2. **Consult for validation** - Cross-check findings via `/ask-claude` or `/ask-gemini`
3. **Critically evaluate** - Never blindly adopt external findings
4. **Graceful fallback** - Never block if external CLIs unavailable
