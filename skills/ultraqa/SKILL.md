...
Use `omghc_state` MCP tools for UltraQA lifecycle state.
...
User can cancel by clearing the state file:
```
...
When goal is met OR max cycles reached OR exiting early, call:

```
state_clear({mode: "ultraqa"})
```

Use the `omghc_state` MCP cleanup rather than deleting files directly.

## Forward-compat note

In Copilot CLI v1.0.40, file-based hooks DO NOT FIRE (R-hooks-not-wired). UltraQA state persists across sessions via the `omghc_state` MCP server, but automatic resume on session restart depends on the `sessionEnd` hook firing. Until Copilot wires hook execution, run `omghc continue` manually after restarting a session, or invoke `$ultraqa` again to resume from persisted state.
