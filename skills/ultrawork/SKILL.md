...
6. **Run dependent tasks sequentially**: Wait for prerequisites before launching dependent work.
...
Use `omghc_state` MCP tools for ultrawork lifecycle state.
...
- `node --test dist/scripts/__tests__/copilot-native-hook.test.js` passes
...
```
Shared-file edit in progress across `src/scripts/copilot-native-hook.ts` and its test → keep implementation local.
