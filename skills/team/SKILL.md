...
`$team` is the tmux-based parallel execution mode for OMGHC. It starts real worker Copilot and/or Claude CLI sessions in split panes and coordinates them through `.omghc/state/team/...` files plus CLI team interop (`omghc team api ...`) and state files.

This skill is operationally sensitive. Treat it as an operator workflow, not a generic prompt pattern. In Copilot CLI App or plain outside-tmux sessions, do not present `$team` / `omghc team` as directly available; launch OMGHC CLI from shell first, or stay on the nearest app-safe surface until the user explicitly wants the tmux runtime.
...
- Use **`omghc team`** when you need durable tmux workers, shared task state, mailbox/dispatch coordination, worktrees, explicit lifecycle control, or long-running parallel execution that must survive beyond one local reasoning burst.
...
1. Invoke OMGHC runtime directly with `omghc team ...`
...
If `omghc team` is unavailable, stop with a hard error.

## Invocation Contract

```bash
omghc team [N:agent-type] "<task description>"
```

Examples:

```bash
omghc team 3:executor "analyze feature X and report flaws"
omghc team "debug flaky integration tests"
omghc team "ship end-to-end fix with verification"
```

### Team-first launch contract

`omghc team ...` is now the canonical launch path for coordinated execution.
...
- **Canonical launch:** use plain `omghc team ...` / `$team ...` for coordinated workers.
- **Verification ownership:** keep one lane focused on tests, regression coverage, and evidence before shutdown.
- **Escalation:** start a separate `omghc ralph ...` / `$ralph ...` only when a later manual follow-up still needs a persistent single-owner fix/verification loop.
- **Deprecation:** `omghc team ralph ...` has been removed. Use plain `omghc team ...` for team execution or run `omghc ralph ...` separately when you explicitly want a later Ralph loop.
...
OMGHC_TEAM_WORKER_CLI=claude omghc team 2:executor "update docs and report"

# Mixed team (worker 1 = Copilot, worker 2 = Claude)
OMGHC_TEAM_WORKER_CLI_MAP=copilot,claude omghc team 2:executor "split doc/code tasks"

# Auto mode: Claude is selected when worker launch args/model contains 'claude'
OMGHC_TEAM_WORKER_CLI=auto OMGHC_TEAM_WORKER_LAUNCH_ARGS="--model claude-..." omghc team 2:executor "run mixed validation"
```
...
3. `omghc` command resolves to the intended install/build
4. If running repo-local `node bin/omghc.js ...`, run `npm run build` after `src` changes
...
If duplicates exist, remove extras before `omghc team` to prevent HUD ending up in worker stack.

## Pre-context Intake Gate

Before launching `omghc team`, require a grounded context snapshot:

1. Derive a task slug from the request.
2. Reuse the latest relevant snapshot in `.omghc/context/{slug}-*.md` when available.
3. If none exists, create `.omghc/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) with:
   - task statement
   - desired outcome
...
For simple read-only brownfield lookups during intake, follow active session guidance: when `USE_OMGHC_EXPLORE_CMD` is enabled, prefer `omghc explore` with narrow, concrete prompts; otherwise use the richer normal explore path and fall back normally if `omghc explore` is unavailable.
...
- include an explicit launch hint (`omghc team N "<task>"` / `$team N "<task>"`) for the coordinated team run; mention a later separate Ralph follow-up only when genuinely needed
...
`omghc team` currently performs:

1. Parse args (`N`, `agent-type`, task)
2. Sanitize team name from task text
3. Initialize team state:
   - `.omghc/state/team/<team>/config.json`
   - `.omghc/state/team/<team>/manifest.v2.json`
   - `.omghc/state/team/<team>/tasks/task-<id>.json`
4. Compose team-scoped worker instructions file at:
   - `.omghc/state/team/<team>/worker-agents.md`
   - Uses project `AGENTS.md` content (if present) + worker overlay, without mutating project `AGENTS.md`
5. Resolve canonical shared state root from leader cwd (`<leader-cwd>/.omghc/state`)
6. Split current tmux window into worker panes
7. Launch workers with:
   - `OMGHC_TEAM_WORKER=<team>/worker-<n>`
   - `OMGHC_TEAM_STATE_ROOT=<leader-cwd>/.omghc/state`
   - `OMGHC_TEAM_LEADER_CWD=<leader-cwd>`
   - worker CLI selected by `OMGHC_TEAM_WORKER_CLI` / `OMGHC_TEAM_WORKER_CLI_MAP` (`copilot` or `claude`)
...
- Workers may run in separate git worktrees (`omghc team --worktree[=<name>]`) while sharing one team state root
...
- Submit routing uses this CLI resolution order per worker trigger:
  ...
  2) `OMGHC_TEAM_WORKER_CLI_MAP` entry for that worker index,
  3) fallback `OMGHC_TEAM_WORKER_CLI` / auto detection.
...
1. Explicit worker model in `OMGHC_TEAM_WORKER_LAUNCH_ARGS`
2. Inherited leader `--model` flag
3. Low-complexity default from `OMGHC_DEFAULT_SPARK_MODEL` (legacy alias: `OMGHC_SPARK_MODEL`) when 1+2 are absent and team `agentType` is low-complexity

Default-model rule:
- Do **not** assume a frontier or spark model from recency or model-family heuristics.
- Use `OMGHC_DEFAULT_FRONTIER_MODEL` for frontier-default guidance.
- Use `OMGHC_DEFAULT_SPARK_MODEL` for spark/low-complexity worker-default guidance.
...
- When the leader assigns teammate roles/tasks, OMGHC allocates **per-worker reasoning effort dynamically** from the resolved worker role (`low`, `medium`, `high`).
- Explicit launch args still win: if `OMGHC_TEAM_WORKER_LAUNCH_ARGS` already includes `-c model_reasoning_effort=...`, that explicit value overrides dynamic allocation for every worker.
...
2. Monitor task and worker progress with runtime/state tools first (`omghc team status <team>`, `omghc team resume <team>`, mailbox/state files)
3. Wait for terminal task state before shutdown:
   - `pending=0`
   - `in_progress=0`
   - `failed=0` (or explicitly acknowledged failure path)
4. Only then run `omghc team shutdown <team>`
...
```bash
sleep 30 && omghc team status <team-name>
```

Repeat that check while the team stays active, or use `omghc team await <team-name> --timeout-ms 30000 --json` when event-driven waiting is a better fit.

If the leader gets a stale/team-stalled nudge, immediately run `omghc team status <team-name>` before taking any manual intervention.
...
1. Use `omghc team ...` runtime lifecycle commands for orchestration.
2. Use `omghc team api ... --json` for mailbox/task mutations.
3. Verify delivery via mailbox/state evidence (`mailbox/*.json`, task status, `omghc team status`).
...
- `shutdown`: graceful shutdown request, then cleanup (deletes `.omghc/state/team/<team>`)

## Data Plane and Control Plane

### Control Plane

- tmux panes/processes (`OMGHC_TEAM_WORKER` per worker)
- leader notifications via `tmux display-message`

### Data Plane

- `.omghc/state/team/<team>/...` files
- Team mailbox files:
- `.omghc/state/team/<team>/mailbox/leader-fixed.json`
- `.omghc/state/team/<team>/mailbox/worker-<n>.json`
- `.omghc/state/team/<team>/dispatch/requests.json` (durable dispatch queue; hook-preferred, fallback-aware)
...
- `.omghc/state/team/<team>/manifest.v2.json`
- `.omghc/state/team/<team>/tasks/task-<id>.json`
- `.omghc/state/team/<team>/workers/worker-<n>/identity.json`
- `.omghc/state/team/<team>/workers/worker-<n>/inbox.md`
- `.omghc/state/team/<team>/workers/worker-<n>/heartbeat.json`
- `.omghc/state/team/<team>/workers/worker-<n>/status.json`
- `.omghc/state/team-leader-nudge.json`


## Team Mutation Interop (CLI-first)

Use `omghc team api` for machine-readable mutation/reads instead of legacy `team_*` MCP tools.

```bash
omghc team api <operation> --input '{"team_name":"my-team",...}' --json
```

Examples:

```bash
omghc team api send-message --input '{"team_name":"my-team","from_worker":"worker-1","to_worker":"leader-fixed","body":"ACK"}' --json
omghc team api claim-task --input '{"team_name":"my-team","task_id":"1","worker":"worker-1"}' --json
omghc team api transition-task-status --input '{"team_name":"my-team","task_id":"1","from":"in_progress","to":"completed","claim_token":"<token>"}' --json
```
...
- Send ACK to `leader-fixed` mailbox via `omghc team api send-message --json`
- Claim/transition/release task lifecycle via `omghc team api <operation> --json`
...
Use only after checking `omghc team status <team>` and mailbox/state evidence:

1. Capture pane tail to confirm current worker state:
   - `tmux capture-pane -t %<worker-pane> -p -S -120`
   - If a larger-tail read or bounded summary would help, prefer explicit opt-in inspection via `omghc sparkshell --tmux-pane %<worker-pane> --tail-lines 400` before improvising extra tmux commands.
...
2. `.omghc/state/team/<team>/mailbox/leader-fixed.json` exists
3. Worker skill loaded and `omghc team api send-message --json` called
4. Task-id mismatch not blocking worker flow

### Worker logs `omghc team api ... ENOENT` (or legacy `team_send_message ENOENT` / `team_update_task ENOENT`)

Meaning:
- Team state path no longer exists while worker is still running.
- Typical cause: leader/manual flow ran `omghc team shutdown <team>` (or removed `.omghc/state/team/<team>`) before worker finished.

Checks:

1. `omghc team status <team>` and confirm whether tasks were still `in_progress` when shutdown occurred
2. Verify whether `.omghc/state/team/<team>/` exists
3. Inspect worker pane tail for post-shutdown writes
4. Confirm no external cleanup (`rm -rf .omghc/state/team/<team>`) happened during execution
...
rm -rf .omghc/state/team/<team-name>

# 4) Retry
omghc team 1:executor "fresh retry"
```

Guidelines:

- Do not kill leader pane
- Do not kill HUD pane (`omghc hud --watch`) unless intentionally restarting HUD
...
Use `omghc sparkshell --tmux-pane ...` as an explicit opt-in operator aid for pane inspection and summaries; keep raw `tmux capture-pane` evidence available for manual intervention and proof.

## Programmatic Team Orchestration

Use the `omghc team ...` CLI as the supported team-launch surface. For automation, drive the same CLI flow from scripts or supervising agents rather than relying on a separate MCP runner.

### Supported current surfaces

- **`omghc team ...` CLI** — Primary method for interactive or automated team orchestration. Use this when you want direct tmux-pane visibility or a scriptable launch path.
- **Team state files** — Inspect `.omghc/state/team/<team>/` when you need status, task, or mailbox evidence after launch.

### Cleanup distinction

Two cleanup paths exist and must not be confused:

- `team_cleanup` (**state-server**): Deletes team state **files** on disk (`.omghc/state/team/<team>/`). Use after a team run is fully complete.
- tmux/session cleanup: Use the documented `omghc team` shutdown / cleanup flow when you need to stop worker panes or clean up an interrupted run.

### Automation example

```
1. omghc team 1:executor "fix bugs"
2. omghc team status <team-name>
3. omghc team shutdown <team-name>
