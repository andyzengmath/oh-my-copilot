...
<Purpose>
Deep Interview is an intent-first Socratic clarification loop before planning or implementation. It turns vague ideas into execution-ready specifications by asking targeted questions about why the user wants a change, how far it should go, what should stay out of scope, and what OMGHC may decide without confirmation.
</Purpose>
...
- **Autoresearch (`--autoresearch`)**: same interview rigor as Standard, but specialized for `$autoresearch` mission readiness and `.omghc/specs/` artifact handoff

If no flag is provided, use **Standard**.

<Mode_Flags>
- **`--autoresearch`**: switch the interview into autoresearch-intake mode for `$autoresearch` handoff. In this mode, the interview should converge on a validator-ready research mission, write canonical artifacts under `.omghc/specs/`, and preserve the explicit `refine further` vs `launch` boundary for downstream skill intake.
</Mode_Flags>
</Depth_Profiles>
...
- When session guidance enables `USE_OMGHC_EXPLORE_CMD`, prefer `omghc explore` for simple read-only brownfield fact gathering; keep prompts narrow and concrete, and keep ambiguous or non-shell-only investigation on the richer normal path and fall back normally if `omghc explore` is unavailable.
...
- In attached-tmux Copilot CLI, deep-interview uses `omghc question` as the required OMGHC-owned structured questioning path for every interview round
- When invoking `omghc question` through attached-tmux Bash/tool paths, preserve the leader-pane return target by prefixing the command with `OMGHC_QUESTION_RETURN_PANE=$TMUX_PANE` (or a concrete `%pane` value)
- If you launch `omghc question` in a background terminal, immediately wait for that background terminal to finish and read its JSON answer before scoring ambiguity, asking another round, or handing off
- If the current runtime is outside tmux and cannot render `omghc question`, use the native structured question tool when available; otherwise ask exactly one concise plain-text question and wait for the answer
...
2. Attempt to load the latest relevant context snapshot from `.omghc/context/{slug}-*.md`.
...
5. Save snapshot to `.omghc/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) and reference it in mode state.
...
```json
{
  ...
  "state": {
    ...
    "context_snapshot_path": ".omghc/context/<slug>-<timestamp>.md"
  }
}
```
...
### 2b) Ask the question
Use the surface-appropriate structured questioning path for every interview round. In attached-tmux sessions, use OMGHC-owned structured questioning via `omghc question` (this is the required `AskUserQuestion` equivalent for deep-interview). Outside tmux, use native structured input when available; otherwise ask exactly one concise plain-text question and wait for the answer. Present:
...
`omghc question` payload guidance for interview rounds:
...
1. Write interview transcript summary to:
   - `.omghc/interviews/{slug}-{timestamp}.md`  
     (kept for ralph PRD compatibility)
2. Write execution-ready spec to:
   - `.omghc/specs/deep-interview-{slug}.md`
...
- Decision Boundaries (what OMGHC may decide without confirmation)
- Constraints
...
- **Canonical artifact path:** `.omghc/specs/deep-interview-autoresearch-{slug}.md`
- **Launch artifact bundle:** `.omghc/specs/autoresearch-{slug}/mission.md`, `.omghc/specs/autoresearch-{slug}/sandbox.md`, and `.omghc/specs/autoresearch-{slug}/result.json`
- **Launch artifact directory:** `.omghc/specs/autoresearch-{slug}/`
- **Required artifact sections:**
  ...
  - `Seed Inputs`
  - `Confirmation Bridge`
- **Required launch artifacts under `.omghc/specs/autoresearch-{slug}/`:**
  - `mission.md`
  - `sandbox.md`
  - `result.json`
...
- **Input Artifact:** `.omghc/specs/deep-interview-{slug}.md` (optionally accompanied by the transcript/context snapshot for traceability)
...
- **Expected Output:** Canonical planning artifacts under `.omghc/plans/`, especially `prd-*.md` and `test-spec-*.md`
...
### 2. **`$autopilot`**
- **Input Artifact:** `.omghc/specs/deep-interview-{slug}.md`
...
- Use `omghc question` as the OMGHC-native structured user-input tool for each interview round when an attached tmux renderer is available
- From attached-tmux Bash/tool paths, call it as `OMGHC_QUESTION_RETURN_PANE=$TMUX_PANE omghc question ...` unless an explicit `%pane` return target is already known
- If the current runtime is outside tmux and cannot render `omghc question`, use native structured input when available; otherwise ask exactly one concise plain-text question and wait for the answer
- Use `state_write` / `state_read` for resumable mode state
- If the interview cannot ask a required `omghc question` round, persist the blocker as terminal state with `active: false` and `current_phase: "blocked"`; do not write a terminal blocked phase with `active: true`
- Read/write context snapshots under `.omghc/context/`
- Record whether the oversized-context summary gate is not needed, pending, or satisfied before any scoring or handoff step
- Save transcript/spec artifacts under `.omghc/interviews/` and `.omghc/specs/`
</Tool_Usage>
...
<Final_Checklist>
- [ ] Preflight context snapshot exists under `.omghc/context/{slug}-{timestamp}.md`
...
- [ ] Transcript written to `.omghc/interviews/{slug}-{timestamp}.md`
- [ ] Spec written to `.omghc/specs/deep-interview-{slug}.md`
