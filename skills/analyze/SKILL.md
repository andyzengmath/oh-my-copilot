...
- the request is purely about running the OMGHC tmux team runtime — use `$team` only when OMGHC runtime is active
...
- Use **`$team` only when OMGHC runtime is active** and durable tmux-based coordination is actually needed.
- Do not imply that `$team` is available in plain Copilot CLI/App sessions.
