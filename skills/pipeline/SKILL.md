...
`$pipeline` is the configurable pipeline orchestrator for OMGHC. It sequences stages
through a uniform `PipelineStage` interface, with state persistence and resume support.

## Default Autopilot Pipeline

The canonical OMGHC pipeline sequences:
...
- **team-exec**: Team execution via Copilot CLI workers. Always the OMGHC execution backend.
- **ralph-verify**: Ralph verification loop with configurable iteration count.

## State Management

Pipeline state persists via the ModeState system at `.omghc/state/pipeline-state.json`.
...
- **team**: Pipeline delegates execution to team mode (Copilot CLI workers)
