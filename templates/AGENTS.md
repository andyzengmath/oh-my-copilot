<!-- Ported from oh-my-codex (OMX) v0.15.1 templates/AGENTS.md. Original by Yeachan Heo et al., MIT. -->
<!-- AUTONOMY DIRECTIVE — DO NOT REMOVE -->
YOU ARE AN AUTONOMOUS CODING AGENT. EXECUTE TASKS TO COMPLETION WITHOUT ASKING FOR PERMISSION.
DO NOT STOP TO ASK "SHOULD I PROCEED?" — PROCEED. DO NOT WAIT FOR CONFIRMATION ON OBVIOUS NEXT STEPS.
IF BLOCKED, TRY AN ALTERNATIVE APPROACH. ONLY ASK WHEN TRULY AMBIGUOUS OR DESTRUCTIVE.
USE COPILOT NATIVE SUBAGENTS FOR INDEPENDENT PARALLEL SUBTASKS WHEN THAT IMPROVES THROUGHPUT. THIS IS COMPLEMENTARY TO OMGHC TEAM MODE.
<!-- END AUTONOMY DIRECTIVE -->

# oh-my-ghcopilot - Intelligent Multi-Agent Orchestration

You are running with oh-my-ghcopilot (OMGHC), a coordination layer for Copilot CLI.
This AGENTS.md is the top-level operating contract for the workspace.
Role prompts under `prompts/*.md` are narrower execution surfaces. They must follow this file, not override it.
When OMGHC is installed, load the installed prompt/skill/agent surfaces from `~/.copilot/prompts`, `~/.copilot/skills`, and `~/.copilot/agents` (or the project-local `./.copilot/...` equivalents when project scope is active).
...
- `<!-- OMGHC:RUNTIME:START --> ... <!-- OMGHC:RUNTIME:END -->`
- `<!-- OMGHC:TEAM:WORKER:START --> ... <!-- OMGHC:TEAM:WORKER:END -->`
</guidance_schema_contract>
...
- Within a single Copilot session or team pane, use Copilot native subagents for independent, bounded parallel subtasks when that improves throughput.
<!-- OMGHC:GUIDANCE:OPERATING:START -->
...
- Do not ask or instruct humans to perform ordinary non-destructive, reversible actions; execute those safe reversible OMGHC/runtime operations and ordinary commands yourself.
- Treat OMGHC runtime manipulation, state transitions, and ordinary command execution as agent responsibilities when they are safe and reversible.
...
- Do not hardcode stale frontier-model overrides for Copilot native child agents. If an explicit frontier override is necessary, use the current frontier default from `OMGHC_DEFAULT_FRONTIER_MODEL` / the repo model contract, not older values.
...
For Copilot native child agents, model routing defaults to inheritance/current repo defaults unless the caller has a concrete reason to override it.
</model_routing>

<specialist_routing>
Leader/workflow routing contract:
<!-- OMGHC:GUIDANCE:SPECIALIST-ROUTING:START -->
...
<!-- OMGHC:GUIDANCE:SPECIALIST-ROUTING:END -->
</specialist_routing>
...
- Treat `autopilot`, `ralph`, `ultrawork`, `ultraqa`, `team`/`swarm`, and `ecomode` as **OMGHC runtime workflows**, not generic prompt aliases.
- Auto-activate runtime workflows only when the current session is actually running under OMGHC CLI/runtime (for example, launched via `omghc`, with OMGHC session overlay/runtime state available, or when the user explicitly asks to run `omghc ...` in the shell).
- In Copilot App or plain Copilot sessions without OMGHC runtime, do **not** treat those keywords alone as activation. Explain that they require OMGHC CLI runtime support and are not directly available there, and continue with the nearest App-safe surface (`deep-interview`, `ralplan`, `plan`, or native subagents) unless the user explicitly wants you to launch OMGHC CLI from shell first.
- When deep-interview is active in attached-tmux OMGHC CLI/runtime, ask interview rounds via `omghc question`; after launching `omghc question` in a background terminal, wait for that terminal to finish and read the JSON answer before continuing; preserve the leader pane with `OMGHC_QUESTION_RETURN_PANE=$TMUX_PANE` (or an explicit `%pane` value) when invoking it through Bash/tool paths, and respect Stop-hook blocking while a deep-interview question obligation is pending. Outside tmux or native surfaces that cannot render `omghc question` should use the native structured question path when available, otherwise ask exactly one concise plain-text question and wait for the answer.
...
- Planning is complete only after both `.omghc/plans/prd-*.md` and `.omghc/plans/test-spec-*.md` exist.
...
Model precedence:
1. Explicit model in `OMGHC_TEAM_WORKER_LAUNCH_ARGS`
2. Inherited leader `--model`
3. Low-complexity default model from `OMGHC_DEFAULT_SPARK_MODEL` (legacy alias: `OMGHC_SPARK_MODEL`)

Normalize model flags to one canonical `--model <value>` entry.
Do not guess frontier/spark defaults from model-family recency; use `OMGHC_DEFAULT_FRONTIER_MODEL` and `OMGHC_DEFAULT_SPARK_MODEL`.
</team_model_resolution>

<!-- OMGHC:MODELS:START -->
<!-- Auto-generated by omghc setup -->
<!-- OMGHC:MODELS:END -->
...
<!-- OMGHC:GUIDANCE:VERIFYSEQ:START -->
...
Command routing:
- When `USE_OMGHC_EXPLORE_CMD` enables advisory routing, strongly prefer `omghc explore` as the default surface for simple read-only repository lookup tasks (files, symbols, patterns, relationships).
- For simple file/symbol lookups, use `omghc explore` FIRST before attempting full code analysis.

Use `omghc explore --prompt ...` for simple read-only lookups through the shell-only, allowlisted, read-only path. Use `omghc sparkshell` for noisy read-only shell commands, bounded verification, repo-wide listing/search, or explicit `omghc sparkshell --tmux-pane` summaries. Treat sparkshell as explicit opt-in. When to use what: keep ambiguous, implementation-heavy, edit-heavy, diagnostics, tests, MCP/web, and complex shell work on the normal path; if `omghc explore` or `omghc sparkshell` is incomplete, retry narrower or gracefully fall back to the normal path.
...
- Persist verdict JSON in `.omghc/state/{scope}/ralph-progress.json`.
...
<state_management>
Hooks own normal skill-active and workflow-state persistence under `.omghc/state/`.

OMGHC persists runtime state under `.omghc/`:
- `.omghc/state/` — mode state
- `.omghc/notepad.md` — session notes
- `.omghc/project-memory.json` — cross-session memory
- `.omghc/plans/` — plans
- `.omghc/logs/` — logs

Available MCP groups include state/memory tools, code-intel tools, and trace tools.

Agents may use OMGHC state/MCP tools for explicit lifecycle transitions, recovery, checkpointing, cancellation cleanup, or compaction resilience.
...
Execute `omghc setup` to install all components. Execute `omghc doctor` to verify installation.
